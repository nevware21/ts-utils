...
const rollupConfigFactory = (isMinified, path, format = "iife", postfix = "") => {
    const taskRollupConfig = {
        input: `lib/dist-esm/index.js`,
        output: {
            file: `lib/dist/${path}/${outputName}${postfix}.js`,
            banner: banner,
            format: format,
    ...
    if (isMinified) {
        taskRollupConfig.output.file = `lib/dist/${path}/${outputName}${postfix}.min.js`;
...
const polyfillRollupConfigFactory = (isMinified, format = "iife", postfix = "") => {
    const taskRollupConfig = {
        input: `lib/dist-esm/polyfills.js`,
        output: {
            file: `lib/bundle/${polyFillOutputName}${postfix}.js`,
    ...
    if (isMinified) {
        taskRollupConfig.output.file = `lib/bundle/${polyFillOutputName}${postfix}.min.js`;
        taskRollupConfig.plugins.push(
            uglify3({
                ie8: true,
                toplevel: true,
